...
if (islemKodu==154){
  ...
  for (Forix=0;Forix<=7;Forix++){ // Cihaz Ismimiz Için 8 Bytelik Yer Ayiriyoruz [MyKey] Gibi.
   Okunanveri=Flash_Read_8(0x7204 +Forix); // 0xC004 Adresinden byte Byte Veri okuyoruz.
 ...
 if (islemKodu==155)
  {
   ReadData(0x7200, 0x7400, (uint32_t *)CihazAyarBuffer);
  Flash_Sil(0x7200,1);
    for( Forix=0;Forix<=7;Forix++){
     CihazAyarBuffer[Forix+4]=buffer[Forix+1];
   }

  WriteData(0x7200, 0x7400, (uint32_t *)CihazAyarBuffer);
   HexDosyaAdiEslesti=0;
  ...
  if (islemKodu==151)
    {
       Flash_Sil(0x00000000UL,57 // Program 57 Sayfadan Olusuyor.Dolayisi ile Veri Yazmadan Önce 57 Sayfayi Temizliyoruz.);
 ...
 else if (islemKodu==153)
  {
  ReadData(0x7200, 0x7400, (uint32_t *)CihazAyarBuffer); // Önce Flasdaki Veriyi Okuyup Bir Liste Aliyoruz.
  Flash_Sil(0x7200,1); // Cihazin Ayar Sayfasini siliyoruz.
    CihazAyarBuffer[0]=0; // Boot Adresinin tutuldugu alani 0 Yapiyoruz. Bootdan çikmasi için.
  WriteData(0x7200, 0x7400, (uint32_t *)CihazAyarBuffer); //Sonra Bu ayarlari Tekrar Yaziyoruz..
  GidenData[1]=1; // Boot Yazma Tamamlandi.
  EP2_Handler();
   uint32_t ix;
   CLK_SysTickLongDelay(10000);
    (*((volatile unsigned int *)(&((SYS_T *) (((uint32_t)0x40000000UL) + 0x00000UL))->RSTSTS))=(3));//clear bit
   ix = (((FMC_T *) (((uint32_t)0x40000000UL) + 0x0C000UL))->ISPCTL & 0xFFFFFFFC);
    (*((volatile unsigned int *)(&((FMC_T *) (((uint32_t)0x40000000UL) + 0x0C000UL))->ISPCTL))=(ix));
   __NVIC_SystemReset();
   return 0;
